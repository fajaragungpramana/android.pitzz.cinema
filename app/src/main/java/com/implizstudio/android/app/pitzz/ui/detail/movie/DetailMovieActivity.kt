...
class DetailMovieActivity : BaseActivity<ActivityDetailBinding>() {
    ...
    override fun onCreated(savedInstanceState: Bundle?) {
        ...
        movieId = intent.getIntExtra(Constant.Key.MOVIE_ID, 0)

        getBinding().handle = OnClickHandle(this)
        viewModel.apply {

            isVisibleProgressBar().observe(this@DetailMovieActivity, Observer { getBinding().isProgressBarVisible = it })

            isMovieFavorite(movieId).observe(this@DetailMovieActivity, Observer {
                iv_favorite.apply { if (it) load(R.drawable.ic_added_favorite) else load(R.drawable.ic_add_favorite) }
            })

        }

        srl_detail.setOnRefreshListener { viewModel.isRefreshing(movieId).observe(this, Observer { srl_detail.isRefreshing = it }) }
    ...
    override fun onResume() {
        ...
        viewModel.getMovie(movieId)?.observe(this, Observer { movie ->
            ...
            rv_production.adapter = ProductionAdapter(this, movie.productions)

            iv_favorite.setOnClickListener { viewModel.favoriteListener(BaseEntity.Movie(null, movie.id, movie.poster, movie.title)) }
        })
